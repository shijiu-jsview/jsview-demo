...
const TextDiv = ({style, img, text, top, inx, callback}) => {
    return (
        text===null?
        <div key={inx} style={{ ...style.item_bg_style, top:top, transition:"top 1.5s linear" }} onTransitionEnd={callback}>
            ...
            <div style={{ ...style.item_rose_style, backgroundImage:img?img:roseImg}}/>
...
class MessageLayer extends FocusBlock {
    ...
    _copyRenderList = () => {
        let oldPoolList = this.state.textPoolList;
        let oldList = this.state.textList;
                ...
                <div style={ this._style.bg_style }>
                    {
                        this.state.textList.map((e, inx) =>{
                            return <TextDiv key={e.key} style={this._style} img={e.img} text={e.text} top={(inx-1)*47} inx={e.key} callback={(parseInt(this.state.animationCnt/7)*7)===e.key?this._onAnmationEnd:null}/> 
                        })
                    }
                </div>
            </span>     
