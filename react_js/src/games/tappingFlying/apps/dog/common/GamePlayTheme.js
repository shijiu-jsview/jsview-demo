...
if (!window.GameSource["ending_btn.json"]) {
  const config_json = require("../assets/atlas/ending_btn.json");
  window.GameSource["ending_btn.json"] = Game.convertToSpriteInfo(config_json);
}

if (!window.GameSource["en.json"]) {
  const config_json = require("../assets/atlas/en.json");
...
const GamePlayTheme = {
  ...
  gameover: {
    ...
    content: {
      ...
      continueBtn: {
        style: {
          left: (1280 - window.GameSource["en.json"].frames[2].target.w) / 2,
          top: (720 - window.GameSource["en.json"].frames[2].target.h) / 2 + 134 + 40,
