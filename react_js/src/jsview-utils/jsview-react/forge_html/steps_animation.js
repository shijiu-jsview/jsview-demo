...
class StepsAnimation extends Forge.AnimationDelegate {
	...
	_NextStep() {
		...
		// 校验动画是否停止
		let on_end = (this._NextStepIndex == this._StepsCount);
		...
		// 描画下一步动画
		let animation = this._CreateOneStepKeyFrame(this._NextStepIndex);
		this._KeyFrameName = animation.name;
		getStaticFrameControl().insertRule(animation.keyFrameString);

		let style_animation = animationToStyle({
	...
	_PerformAnimationEnd(on_end) {
		...
		let progress = this._NextStepIndex / this._StepsCount;

		if ((this.enableFlags & Forge.AnimationEnable.KeepTransform) != 0) {
			let frozen_transform = this._GetFrozenTransform(this._NextStepIndex - 1);
...
Forge.TranslateStepAnimation = class extends StepsAnimation {
	...
	_SplitSteps() {
		...
		let x_step_distance = (this.endX - this.startX) / steps_count;
		let y_step_distance = (this.endY - this.startY) / steps_count;
	...
	_CreateOneStepKeyFrame(next_step_index) {
		...
		// 计算动画结束位置
		let end_x = this._StepsArray[next_step_index].targetX;
		let end_y = this._StepsArray[next_step_index].targetY;

		let keyframe_name = "_ForgeStepAnim_TL_" + (sKeyFrameTokenGenerator++);
	...
	_GetFrozenTransform(current_index) {
		let transform = "translate3d("
				+ this._StepsArray[current_index].targetX + "px,"
				+ this._StepsArray[current_index].targetY + "px,0)";
