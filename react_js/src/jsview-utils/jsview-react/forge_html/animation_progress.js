...
const __parseTransform = (transform) => {
...
function _BuildTracer(layout_view) {
  const children = layout_view.Element.children;
...
class AnimationProgress {
  ...
  GetProgress() {
    const s = window.getComputedStyle(this._TracerDiv);
    if (s.transform !== "none") {
      const trans_values = __parseTransform(s.transform);
  ...
  _BuildTraceKeyFrame(starter_progress/* 0~1 */, token, dynamic_count) {
    const keyframe_name = `_AnimateProgress_${token}_${dynamic_count}`;
    const keyframe_control = getStaticFrameControl();
    const animate_progress = `@keyframes ${keyframe_name}
...
class AnimationGroupProgress {
  ...
  GetProgress() {
    ...
    if (s.transform !== "none") {
      const trans_values = __parseTransform(s.transform);
      if (trans_values !== null) {
        const p = (trans_values.params[4] / (this._StepsTotal * 100)); // type is matrix...
  ...
  _BuildTraceKeyFrameGroup(token, dynamic_count) {
    ...
    for (let i = 0; i < this._KeyFrameNameArray.length; i++) {
      const step_name = (i === this._KeyFrameNameArray.length - 1 ? "finalStep" : `${i}`);
      const keyframe_name = `_AnimateGroupProgress_${token}_${dynamic_count}_Cnt_${step_name}`;
      const animate_progress = `@keyframes ${keyframe_name}
  ...
  _RemoveTraceKeyFrameGroup() {
    const keyframe_control = getStaticFrameControl();
    this._KeyFrameNameArray.forEach((name) => {
      keyframe_control.removeRule(name);
    });
  }
