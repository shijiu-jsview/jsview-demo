class PlatformUtils{
    ...
    static GetCursorOffset(str, max_rect, font_params, text_attr, line_height, pos_x, pos_y) {
        ...
        if (font_params.alignment == "right") {
            let width = this.getTextPixelWith(str, bold+" "+italic+" "+font_params.size+"px "+font_params.font);
            pos_x = pos_x - (max_rect.width - width);
            if (pos_x < 0) {
                pos_x = 0;
            }
        }
    ...
    static GetCursorPosition(str, max_rect, font_params, text_attr, line_height, cursor_offset) {
        ...
        if (font_params.alignment == "right") {
            //总长度
            let total_width = this.getTextPixelWith(str, bold+" "+italic+" "+font_params.size+"px "+font_params.font);
