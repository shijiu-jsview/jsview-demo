...
Forge.EnableLog = function(level) {
	...
	if (level > 0)
		Forge.LogF = console.error.bind(console);

	if (level > 1)
		Forge.LogE = console.error.bind(console);

	if (level > 2)
		Forge.LogW = console.warn.bind(console);

	if (level > 3)
		Forge.LogI = console.log.bind(console);

	if (level > 4)
		Forge.LogD = console.debug.bind(console);
}
