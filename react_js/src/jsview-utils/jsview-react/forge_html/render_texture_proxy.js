...
class RenderTextureDelegateManager {
	...
	CreateTexture() {
		var texture = new Forge.RenderTextureDelegate(this, this._IdTokenGenerator++);
		texture.DoRef();
		return texture;
	};
...
class RenderTextureDelegate {
	constructor(manager, assigned_id_token) {
		...
		var enable_trace = Forge.ForgeDebug.GetTextureTracer();
		if (enable_trace) {
			var error_for_stack = new Error();
	...
	ReadSyncInfo() {
		var info = this._SyncingResourceInfo;
		this._SyncingResourceInfo = null;
		return info;
	};

	RegisterOnloadCallback(callback) {
		var id_token = this._OnloadCallbackIdGenerator++;
	...
	InvokeOnloadCallbacks() {
		if (this._OnloadCallbacksSize > 0) {
			for (var a in this._OnloadCallbacks) {
