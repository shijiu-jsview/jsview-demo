...
class WebpData extends ApicData {
    decode(array_buffer) {
        ...
        for (let f = 0; f < frames.length; f++) {
            ...
            let width = [0];
            let frame = frames[f];
            let rgba = webp_decoder.WebPDecodeRGBA(webp_data, frame['src_off'], frame['src_size'], width, height);
            frame['rgba'] = rgba;
            ...
            if (!header) {
                ...
                if (blend) {
                    let oldimagedata_ = this._Context.getImageData(frame['offset_x'], frame['offset_y'], width[0], height[0]);
            ...
            if (frames.length == 1) {
                ...
                this._Context.putImageData(imagedata, frame['offset_x'], frame['offset_y']);
            }

            frame['frameData'] = frame['rgba'] ? (header ? this._Context.getImageData(0, 0, header['canvas_width'], header['canvas_height']).data : rgba) : null;
            if (frame['dispose'] == 1) {
                this._Context.clearRect(frame['offset_x'], frame['offset_y'], width[0], height[0]);
            }
        }
    ...
    toImage(frame_index, canvas, canvas_ctx) {
        ...
        let height = this._DecodedData["header"] ? this._DecodedData["header"]["canvas_height"] : frame["imgheigth"];
        canvas.width = width;
...
class GifData extends ApicData {
    decode(array_buffer) {
        let gif = parseGIF(array_buffer);
        this._DecodedData = decompressFrames(gif, true);
        this.LoopCount = 0;
...
class JsvApic extends React.Component {
                ...
                if (main_view && main_view.ChildViews.length > 0) {
                    ...
                    if (target_view.TextureSetting && target_view.TextureSetting.Texture && target_view.TextureSetting.Texture.RenderTexture) {
                        ...
                        if (this.props.onStart) {
                            target_view.TextureSetting.Texture.registerOnStart(this.props.onStart);
                        }
                        if (this.props.onEnd) {
    ...
    componentDidMount() {
        if (!window.JsView) {
            let canvas = originDocument.getElementById(this._CanvasId);
            if (canvas) {
    ...
    loadWebp(url, canvas) {
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                this._Viewer = new Viewer(apic_data, canvas, listener, this.props.autoPlay);
            }
        };
        xhr.send();
    }
