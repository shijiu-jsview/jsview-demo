...
class JsvTouchContainer extends React.Component {
	...
	componentDidMount() {
		if (this._touchRef && this._touchRef.jsvMaskView) {
			let view = this._touchRef.jsvMaskView;
			let view_lp = view.GetLayoutParams();
			let page_width = this.props.flingPageWidth;
			let page_edge = this.props.flingPageEdge;
			...
			var drag_setting = new Forge.DragSetting(direction, this.props.triggerMovedDistance, false,
	...
	render() {
		...
		return (
			<div key="containerPos"style={{left:left, top:top,
