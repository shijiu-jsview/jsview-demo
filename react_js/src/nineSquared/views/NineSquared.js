...
class NineSquared extends FocusBlock{
    constructor(props) {
        ...
        let translate_info = this._Theme.NineSquared.data[this.awardId];
        let style = {left:translate_info.left,top:translate_info.top,
            width:translate_info.width,height:translate_info.height,
    ...
    _StartGame() {
        ...
        let length = this._Theme.NineSquared.data.length;
        let awardID = parseInt(Math.random()*length);//TODO ，后台请求
        //生成step

        let step = this.state.round * length+awardID+1;//一共几周*总奖品数
        let duration = 5;
        let animation_name = "translate_to_"+(step)+""+parseInt((Math.random()*10));
        let animation = animation_name + " "+duration+"s steps(1,start)";
        let pre_transloate_name = CommonApi.getNineSquaredTranslateName();
        ...
        if (ballRunKeyframes != null) {
            ...
            let used_time = 0;
            let cubicbezier = CommonApi.cubicbezier(0.25,0.1,0.25,1);
            for(let i=this.PreAwardId; i<step; i++) {
                let translate = this._Theme.NineSquared.data[(i)%length];
                let time = cubicbezier.solve(((i-this.PreAwardId)/(step-this.PreAwardId)))*100;
                used_time = parseInt((time*1000))/1000;
                if (i === 0) {
                    used_time = 0;
                } else if (i === step - 1) {
                    used_time = 100;//对齐
        ...
        let translate_info = this._Theme.NineSquared.data[this.awardId];
        let style = {width:translate_info.width,height:translate_info.height,
            backgroundImage:this._Theme.NineSquared.moveFocusBg.backgroundImage,
            animation:animation}
        this.setState({
            moveStyle:style,
