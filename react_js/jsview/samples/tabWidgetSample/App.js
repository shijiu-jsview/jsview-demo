...
class MainScene extends FocusBlock {
  ...
  _RenderFocus(item) {
    const x = -5 + (item.blocks.w - 10 - (item.blocks.w - 10) * 1.05) / 2;
    const y = -5 + (item.blocks.h - 10 - (item.blocks.h - 10) * 1.05) / 2;
    return (
            <div style={{
              animation: "focusScale 0.2s",
              ...
              left: x,
              top: y,
              width: (item.blocks.w - 10) * 1.05,
              height: (item.blocks.h - 10) * 1.05
            }}>
            </div>
    );
  }
  ...
  _RenderBlur(item, callback) {
    return (
            <div style={{
              animation: "blurScale 0.2s",
              backgroundImage: `url(${item.img})`,
              borderRadius: '8px 8px 8px 8px',
              left: -5,
              top: -5,
              width: item.blocks.w - 10,
              height: item.blocks.h - 10
            }}
